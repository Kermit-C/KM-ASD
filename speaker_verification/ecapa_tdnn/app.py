...
def launch_gradio():
    inputs = [
        gr.inputs.Audio(source="microphone", type="filepath", label="说话人 #1", optional=True),
        gr.inputs.Audio(source="microphone", type="filepath", label="说话人 #2", optional=True)
    ]
    description = (
        "声纹识别（说话人确认）演示，用于判断两段音频的说话人是否为同一人，支持中英文。 "
        " "
    )
    ...
    iface = gr.Interface(fn=SpeakerVerification, inputs=inputs, outputs="text", examples=examples,
                        title="声纹识别", description=description)
