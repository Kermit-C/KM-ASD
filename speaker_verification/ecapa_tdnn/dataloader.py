...
def loadWAV(filename, max_frames, segment=1, evalmode=False, num_eval=10):
    ...
    if len(audio.shape) == 2:
        ...
        audio = audio[::3]  # 下采样 16khz
    audiosize = audio.shape[0]
    if audiosize <= max_audio:  # 给不足 max_frams 的 padding
        shortage = max_audio - audiosize + 1
        audio = np.pad(
            audio, (0, shortage), "wrap"
        )  # 0 是前面的 padding，后面 padding shortage 本身的循环
        audiosize = audio.shape[0]

    if evalmode:
        startframe = np.linspace(
            0, audiosize - max_audio, num=num_eval
        )  # 测试的话分为 10 段
    else:
        startframe = np.array(
            [
...
class AugmentWAV(object):
    ...
    def additive_noise(self, noisecat, audio):
        ...
        for noise in noiselist:
            noiseaudio = loadWAV(
                noise, self.max_frames, segment=self.segment, evalmode=False
            )
...
def built_train_loader(cfg):
    if cfg.DATA.LOADER == "dataloader":
        train_dataset = train_dataset_loader(
            ...
            musan_path=cfg.DATA.MUSAN_PATH,
...
if __name__ == "__main__":
    ...
    # print(audio.shape)
    train_dataset = train_dataset_loader(
        train_list="data/train_list_vox.txt",
        augment=True,
