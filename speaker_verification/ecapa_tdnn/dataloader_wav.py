...
# (1, 47920)
def loadWAV_wav(filename, max_frames, segment=1, evalmode=False, num_eval=10):
...
class AugmentWAV_wav(object):
    ...
    def additive_noise(self, noisecat, audio):
        ...
        for noise in noiselist:
            noiseaudio = loadWAV_wav(noise, self.max_frames, segment=self.segment, evalmode=False)
...
class train_dataset_wav_loader(Dataset):
    def __init__(self, train_list, train_path, augment, musan_path, rir_path, max_frames, nPerSpeaker):
        ...
        self.augment_wav = AugmentWAV_wav(musan_path=musan_path, rir_path=rir_path, max_frames=max_frames, segment=nPerSpeaker)
    ...
    def __getitem__(self, index):
        audio = loadWAV_wav(self.data_list[index], self.max_frames, evalmode=False, segment=self.nPerSpeaker)
        if self.augment:
...
class test_dataset_wav_loader(Dataset):
    ...
    def __getitem__(self, index):
        audio = loadWAV_wav(os.path.join(self.test_path, self.test_list[index]), self.max_frames, evalmode=True, num_eval=self.num_eval)
...
def built_train_wav_loader(cfg):
    if cfg.DATA.LOADER == "dataloader_wav":
        train_dataset = train_dataset_wav_loader(train_list=cfg.DATA.TRAIN_LIST, train_path=cfg.DATA.TRAIN_PATH,
...
if __name__ == "__main__":
    ...
    # print(audio.shape)
    train_dataset = train_dataset_wav_loader(train_list="data/train_list_vox1.txt", augment=True,
                                 musan_path="data/musan_split", rir_path="data/RIRS_NOISES/simulated_rirs",
                                 max_frames=300, train_path="data/voxceleb1", nPerSpeaker=2)
    train_loader = torch.utils.data.DataLoader(
        train_dataset,
        batch_size=32,
        pin_memory=False,
