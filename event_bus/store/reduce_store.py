...
class ReduceStore:
    def __init__(
        ...
        self.frame_resulted_count_lock = Lock()
    ...
    def is_frame_result_complete(self, request_id: str, frame_count: int) -> bool:
        ...
        is_complete = (
            len(frame_results["frame_face_idx"]) == frame_results["frame_face_count"]
            and all(
    ...
    def is_frame_resulted(self, request_id: str, frame_count: int) -> bool:
        ...
        if len(request_store["frame_results"]) < frame_count:
            return False
        return request_store["frame_results"][frame_count - 1]["is_resulted"]
    ...
    def get_frame_after_all_complete_but_not_resulted(
        ...
        for i in range(frame_count, len(request_store["frame_results"])):
            ...
            if frame_result is None:
                return result
            if frame_result["is_complete"] and not frame_result["is_resulted"]:
                result.append(i)
        return result
